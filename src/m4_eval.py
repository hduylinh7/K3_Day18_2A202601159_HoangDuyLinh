...
def failure_analysis(eval_results: list[EvalResult], bottom_n: int = 10) -> list[dict]:
    ...
    for result in eval_results:
        avg_score = sum([result.faithfulness, result.answer_relevancy,
                         result.context_precision, result.context_recall]) / 4.0
        worst_metric = min(
            ["faithfulness", "answer_relevancy", "context_precision", "context_recall"],
            key=lambda x: getattr(result, x)
        )
    ...
    per_question.sort(key=lambda x: x["score"])
    return per_question[:bottom_n]
